...
#include <franka_example_controllers/fr3/joint_impedance_force_z_down_controller.hpp>
...
JointImpedanceForceZDownController::command_interface_configuration() const {
...
controller_interface::InterfaceConfiguration
JointImpedanceForceZDownController::state_interface_configuration() const {
  controller_interface::InterfaceConfiguration config;
  config.type = controller_interface::interface_configuration_type::INDIVIDUAL;
...
controller_interface::return_type JointImpedanceForceZDownController::update(
  ...
  // External wrench is O_F_ext_hat_K in world frame; convert to downward-positive scalar.
  const double force_down_measured = -robot_state_ptr_->O_F_ext_hat_K[2];
  const double dt = period.seconds();
  const double tau = 1.0 / (2.0 * M_PI * std::max(1e-3, force_filter_cutoff_hz_));
  const double alpha = dt / (tau + dt);
  force_down_filtered_ += alpha * (force_down_measured - force_down_filtered_);

  // Outer-loop Z admittance updates z reference to track desired contact force.
  ...
  // Orientation target: z-axis aligned with world -Z, yaw fixed by initial projected x-axis.
  const Vector3d orientation_error =
      0.5 * (rotation.col(0).cross(rotation_desired_.col(0)) +
             rotation.col(1).cross(rotation_desired_.col(1)) +
             rotation.col(2).cross(rotation_desired_.col(2)));
  ...
  // Cartesian wrench to joint torques: tau_task = J^T * W.
...
CallbackReturn JointImpedanceForceZDownController::on_init() {
  try {
    auto_declare<std::string>("robot_type", "fr3");
    auto_declare<std::string>("arm_prefix", "");

    auto_declare<double>("translational_stiffness.x", 20.0);
    auto_declare<double>("translational_stiffness.y", 20.0);
    auto_declare<double>("translational_stiffness.z", 400.0);

    auto_declare<double>("translational_damping.x", 40.0);
    auto_declare<double>("translational_damping.y", 40.0);
    auto_declare<double>("translational_damping.z", 70.0);

    auto_declare<double>("rotational_stiffness.x", 30.0);
    auto_declare<double>("rotational_stiffness.y", 30.0);
    auto_declare<double>("rotational_stiffness.z", 30.0);

    auto_declare<double>("rotational_damping.x", 6.0);
...
CallbackReturn JointImpedanceForceZDownController::on_configure(
...
CallbackReturn JointImpedanceForceZDownController::on_activate(
    const rclcpp_lifecycle::State& /*previous_state*/) {
  ...
  robot_state_interface_index_ = static_cast<std::size_t>(std::distance(state_interfaces_.cbegin(), it));
  if (!read_robot_state_pointer()) {
    return CallbackReturn::ERROR;
  }
...
void JointImpedanceForceZDownController::update_joint_states() {
  for (int i = 0; i < kNumJoints; ++i) {
...
bool JointImpedanceForceZDownController::read_robot_state_pointer() {
...
void JointImpedanceForceZDownController::initialize_targets() {
  const auto flange_pose_array = franka_robot_model_->getPoseMatrix(franka::Frame::kFlange);
  const Eigen::Map<const Eigen::Matrix4d> flange_pose(flange_pose_array.data());
  ...
  const Eigen::Matrix3d rotation_initial = flange_pose.block<3, 3>(0, 0);

  const Vector3d z_down = Vector3d(0.0, 0.0, -1.0);
  Vector3d x_reference = rotation_initial.col(0);
  x_reference = x_reference - x_reference.dot(z_down) * z_down;
  if (x_reference.norm() < 1e-6) {
    x_reference = Vector3d(1.0, 0.0, 0.0);
  }
  x_reference.normalize();

  Vector3d y_desired = z_down.cross(x_reference);
  if (y_desired.norm() < 1e-6) {
    y_desired = Vector3d(0.0, 1.0, 0.0);
  }
  y_desired.normalize();

  // Desired rotation is fixed at startup: z-axis down and initial horizontal yaw preserved.
  rotation_desired_.col(0) = x_reference;
  rotation_desired_.col(1) = y_desired;
  rotation_desired_.col(2) = z_down;

  // Initialize force filter with current measured downward force.
  force_down_filtered_ = -robot_state_ptr_->O_F_ext_hat_K[2];
}

JointImpedanceForceZDownController::Vector7d JointImpedanceForceZDownController::saturate_torque_rate(
    const Vector7d& tau_desired) const {
...
// NOLINTNEXTLINE
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::JointImpedanceForceZDownController,
